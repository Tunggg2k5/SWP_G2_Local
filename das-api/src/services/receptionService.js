...
async function autoCancelAfterClinicClose(dateText) {
  ...
  await receptionRepository.cancelAppointmentsAfterClose(
    {
...
export async function getDashboard(query) {
  await autoCancelAfterClinicClose(query.date);

  const [appointments, patients, services, consultations, rooms] = await Promise.all([
    receptionRepository.findReceptionAppointments(buildAppointmentQuery(query.date)),
